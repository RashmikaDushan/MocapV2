...
class Cameras:
    def __init__(self):
        """Initializing the camera object array with the specified parameters"""
        load_dotenv()
        filename = os.getenv("CAMERA_PARAMS_IN")
        cam_params = open(filename)
        self.camera_params = json.load(cam_params) # get the camera_params from the json file

        self.cameras = Camera(fps=90, resolution=Camera.RES_SMALL, gain=10, exposure=100) # create a camera object with the specified parameters

        self.num_cameras = len(self.cameras.exposure) # get the number of cameras
        print(f"Number of cameras available: {self.num_cameras}")
        ...
        self.cameras_init = True
    
    def edit_settings(self, exposure, gain):
        """Set the exposure and gain of the camera"""
...
#------------------------ Triangulating points ------------------------#
                    ...
                    errors, object_points, frames = find_point_correspondance_and_object_points(image_points_all_cameras, self.camera_poses, frames)
    ...
    def start_capturing_points(self): # just setting the capturing points flag to true
        self.is_capturing_points = True

    def stop_capturing_points(self): # just setting the capturing points flag to false
        self.is_capturing_points = False

    def start_trangulating_points(self, camera_poses): # setting the capturing points and triangulating points flags , getting the camera poses, set the kalman filter
        self.is_capturing_points = True
        self.is_triangulating_points = True
        self.camera_poses = camera_poses
        self.kalman_filter = KalmanFilter(self.num_objects)

    def stop_trangulating_points(self): # complet of the triangulating points
        self.is_capturing_points = False
        self.is_triangulating_points = False
        self.camera_poses = None

    def start_locating_objects(self): # just setting the locating objects flag to true
        self.is_locating_objects = True

    def stop_locating_objects(self): # just setting the locating objects flag to false
        self.is_locating_objects = False
    
    def get_camera_params(self, camera_num): # return the camera parameters
        return {
    ...
    def set_camera_params(self, camera_num, intrinsic_matrix=None, distortion_coef=None): # mannually set camera params
...
def find_point_correspondance_and_object_points(image_points, camera_poses, frames):
    ...
    for image_points_single_camera in image_points:
        try:
            image_points_single_camera.remove([None, None])
        except:
            pass
...
def make_square(img):
    """Make the original image square with padding and feathering effect"""
    x, y, _ = img.shape
    size = max(x, y)
    new_img = np.zeros((size, size, 3), dtype=np.uint8) # Make a new 2D array
    ax,ay = (size - img.shape[1])//2,(size - img.shape[0])//2
    new_img[ay:img.shape[0]+ay,ax:ax+img.shape[1]] = img # Place the original image in the center applying the padding
